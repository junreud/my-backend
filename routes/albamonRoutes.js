...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const data = await crawlAlbamonController(req, res);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await processBusinessContacts(req, res);
...
router.get(
  '/data',
  asyncHandler(async (req, res) => {
    const data = await getCustomersWithContacts(req, res);
    return sendSuccess(res, data);
  })
);
...
router.delete(
  ...
  handleValidationErrors,
  asyncHandler(async (req, res) => {
    const deleted = await CustomerInfo.destroy({ where: { id: req.params.id } });
    if (!deleted) return sendError(res, 404, '고객 정보를 찾을 수 없습니다.');
...
router.patch(
  ...
  handleValidationErrors,
  asyncHandler(async (req, res) => {
    const result = await updateFavorite(req, res);
    return sendSuccess(res, result);
  })
);
...
router.patch(
  ...
  handleValidationErrors,
  asyncHandler(async (req, res) => {
    const result = await updateBlacklist(req, res);
    return sendSuccess(res, result);
  })
);
